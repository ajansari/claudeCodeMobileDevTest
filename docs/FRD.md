...
| **DR-5** | **The BBB profile URL is staff-entered and staff-owned.** The extension never guesses, searches for, or derives it, and never silently substitutes a different one. **The host the URL points to is validated against a `bbb.org` pattern before the extension will call it** (refined 2026-09-17, Code Review CR-05, ChangeLog DEFINE-017): staff still choose *which* BBB page a customer's URL points to (this refinement does not reverse that ownership principle), but the extension no longer accepts an arbitrary host — closing an SSRF exposure where a staff- or API-writable URL field could otherwise make the tenant issue an outbound call to any address. | PRE-01 Resolved Decisions. Attaching the wrong company's rating to a customer is a serious data-integrity failure. |
...
- **NFR-22 — No BC Privacy Notice is registered or required for this extension's outbound call to
  bbb.org.** Decided explicitly by AJ Ansari, 2026-09-17 (Code Review CR-19, ChangeLog DEFINE-017),
  rather than left as a silent gap: the data sent (a staff-entered public business URL) and received
  (public BBB rating data) is not personal data (NFR-12); no credentials are stored (NFR-13); and
  DR-9 forbids the configuration/registration surface a privacy notice would need. Should this
  extension ever begin sending or receiving anything that is personal data, or add a setup surface,
  this decision must be revisited against BC's `Codeunit "Privacy Notice"` mechanism.
