...
| 50608 | codeunit | `ocpfBbbCustomerSubscribers` | — | M2 | B2 | n/a | Event subscribers only (Standards §10.1). Cascade delete (OQ-6) + rename follow-through (uses `ModifyAll`, CR-04, ChangeLog DEFINE-017). Carries `Permissions = tabledata "ocpfBbbFetchLog" = RMD;` (narrowed from `RIMD`, CR-09, ChangeLog DEFINE-017 — `I` was unused by either subscriber). **Not in FRD §9** — a delivery mechanic for NFR-15 (TDD §3.5). Rename subscriber's necessity is pending AJ Ansari's local verification (CR-04) — not yet removed. |
...
| 50617 | permissionset | `OCPFBBB BBBRI, EDIT` | — | M5 | B1 (created), amended B2–B4 | Read/Write | `Assignable = true`, Caption `'BBB Rating Insights - Edit'`, `IncludedPermissionSets = "OCPFBBB BBBRI, VIEW"`. `tabledata "ocpfBbbFetchLog" = RID` (narrowed from `RIMD`, F-M-1, ChangeLog DEFINE-014 — no user-facing surface (UI or API) can modify a log row; the only code path that does is the rename-tracking subscriber (50608), which runs under its own codeunit-level Permissions grant, pending CR-04's verification of whether that subscriber is even needed — corrected, CR-23, ChangeLog DEFINE-017) + execute on 50606/50607. Credit & Risk only (FR-11/OQ-4). Pairs with `D365 BUS FULL ACCESS`. |
